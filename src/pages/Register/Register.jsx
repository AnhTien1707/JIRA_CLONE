...
import { register } from "../../store/QuanlyUser/thunkAction";
...
const Register = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      .matches(/^[a-z A-Z]+$/,"Tên nhập vào phải là chữ !"),
      email: Yup.string()
      .required('Bạn không được để trống !').email('Bạn phải nhập vào là email !'),
      passWord: Yup.string()
      .required("Bạn không được để trống !")
      .min(5,'Bạn phải nhập tối đa 5 kí tự !'),
      phoneNumber: Yup.string()
      .required("Bạn không được để trống !")
      .matches(/^[0-9]+$/ , "Bạn phải nhập vào là số !")
    }),
    onSubmit: async (values) => {
      try{
        const res = await quanLyUserService.register(values)
